...
function toggleFeature(feature,value){
	...
	//console.log(feature+": " + localStorage.getItem(feature));
}
...
function quotePreview(reference, mode){
	...
	if (document.getElementById(referencedPostNumber) != null){
		...
		if (mode == 1){
			//document.getElementById("quotePreview"+referencedPostNumber).outerHTML = ""; // THIS IS STUPID
			//document.getElementById("quotePreview"+referencedPostNumber).parentNode.removeChild(document.getElementById("quotePreview"+referencedPostNumber)); //THIS WORKS
		...
		if (mode == 0){
			...
			$.get(reference.href, function(data) {
				...
				if(loadedPost==null){
					...
					//console.log(referencedPostNumber.replace('reply','parent'));
				}
				
				//$(div.innerHTML).empty().append(data);
...
function threadUpdater(){
	...
	if (document.getElementById("lastModified").innerHTML == modified){
		...
		for (var i = 0; i < difference; i++){
			...
			//console.log($($(denguson).children('.reply')[0]).attr('class'));
			$($(denguson).children('.reply')[0]).attr('class','reply newPost');
			
			// removed in titlefactory
			if((i==0)&&(postsInTitle==0)){
				$($(denguson).children('.reply')[0]).attr('class','reply newPost unreadMarker');
				$($(denguson).children('.reply')[0]).css('box-shadow','0 -3px red');
			}
			
			//console.log($($(denguson).children('.reply')[0]).attr('class'));
			//document.getElementsByClassName("thread")[0].appendChild(denguson);
			$('.thread').append(denguson);
			//$($($(denguson).children('.reply')[0]).attr('id')).attr('class','reply newPost');
			//document.getElementById($($(denguson).children('.reply')[0]).attr('id')).className=='reply newPost';
		}
		*/
...
function makeReply(post){
	...
	var refLinkInner = document.createElement('span');
