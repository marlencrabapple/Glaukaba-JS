...
var postsAdded = 0;
var numberOfPosts = 0;
var settings = ["qRep", "inlineExpansion", "threadUpdater", "quotePreview", "replyHiding", "threadHiding", "anonymize", "inlineQuote", "expandPosts", "expandThreads", "replyBacklinking", "expandFilename"];
var hasCaptcha = 0;
var postsInTitle = 0;
...
function loadSavedSettings() {
  for (var i = 0; i < settings.length; i++) if (localStorage.getItem(settings[i]) == "false" || localStorage.getItem(settings[i]) == null) document.getElementById(settings[i]).checked = "";
  else document.getElementById(settings[i]).checked = "checked"
}
...
function updaterPrep() {
  if (postsAdded < 1) if (document.body.className == "replypage") {
...
function hideThreadPrep() {
  ...
  for (var i = 0; i < threads.length; i++)(function (e) {
    var parentPost = threads[e].firstElementChild;
    var filesize = $(parentPost).children(".filesize")[0];
    if (!filesize) var filesize = $(parentPost).children(".parentPostInfo")[0];
    var hideButton = document.createElement("a");
    hideButton.innerHTML = "<a class='hidePostButton' href='javascript:void(0)'>[ - ]</a> ";
    hideButton.addEventListener("click", function () {
      hideThread(threads[e],
      parentPost, hideButton)
    });
    $(filesize).before(hideButton);
    if (localStorage.getItem("thread" + parentPost.id.substring(6) + "Hidden") == "true") hideThread(threads[e], parentPost, filesize)
  })(i)
}
...
function hideReplyPrep() {
  ...
  doubledashes.each(function (e) {
    if (localStorage.getItem(this.nextElementSibling.id + "Hidden") == "true") hidePost(this.nextElementSibling.id)
  })
}
...
function threadExpansionPrep() {
  ...
  for (var i = 0; i < links.length; i++) links[i].outerHTML = "<span class='omittedposts processed'><a style='text-decoration: none;' href='javascript:void(0)' onclick=expandThread('" + links[i].previousElementSibling.id + "',0)>+ " + links[i].innerHTML.replace("Reply", "here") + "</a></span>"
}
...
function replyBacklinkingPrep() {
  ...
  for (var i = 0; i < references.length; i++) for (var a = 0; a < postNumbers.length; a++) {
    ...
    if (postNumbers[a].innerHTML.indexOf(referencedPost) != -1) {
      ...
      if (postReferenced.indexOf("reply") != -1) {
        ...
        for (var x = 0; x < postReferenced.length; x++) if (postReferenced[x].indexOf("reply") != -1) postReferenced = postReferenced[x];
        postReferenced = postReferenced.replace("reply", "")
      } else {
        var postReferencedId = "parent" + referencedPost.replace("No.", "");
        postReferenced = postReferenced.split(",");
        for (var x = 0; x < postReferenced.length; x++) if (postReferenced[x].indexOf("parent") != -1) postReferenced = postReferenced[x];
      ...
      if (document.getElementById("backlink" + postReferenced) == null) {
        ...
        if (localStorage.getItem("inlineQuote") == "true") $(backlink).bind("click",

        function (a) {
          inlineQuote(this, this.href, 0, 1);
          a.preventDefault()
        });
        if (backlink.innerHTML.indexOf("inline") == -1) postNumbers[a].parentElement.parentElement.appendChild(backlink)
      }
    }
  }
}
...
function highlightPosts(identifier) {
  if (identifier != undefined) {
    ...
    if (identifier.className == "postername") if (identifier.firstElementChild != null) {
    ...
    } else console.log("Nothing to identify by");
    else if (identifier.className == "postertrip") if (identifier.firstElementChild != null) {
...
function titleFactory(mode) {
  if (mode == 1) {
    ...
    return
  } else if (isOn == 1) if ($(".newPost").last().length) if ($(document).scrollTop() - 100 > $(".newPost").last().offset().top - $(window).height()) {
    postsInTitle = 0;
    document.title = "(" + postsInTitle + ") " + $(".parentPost").children("blockquote").first().text();
...
function expandPost(link) {
  if (link.hash) $.get(link, function (data) {
...
function expandThread(parentDivId, mode) {
  ...
  if (mode == 0) $.get(threadLink, function (data) {
  ...
  else $.get(window.location.href, function (data) {})
}
...
function anonymize() {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    if (names[i].nextElementSibling != null) if (names[i].nextElementSibling.className == "postertrip") names[i].nextElementSibling.outerHTML = ""
  }
  for (var i = 0; i < names2.length; i++) {
    names2[i].innerHTML = "Anonymous";
    if (names2[i].nextElementSibling != null) if (names2[i].nextElementSibling.className == "postertrip") names2[i].nextElementSibling.outerHTML = ""
  }
}
...
function quickReply(refLink, board) {
  ...
  if (document.getElementById("quickReply") == null) {
    ...
    qrAjaxSubmit()
  } else document.getElementById("quickReply").childNodes[2].childNodes[7].childNodes[7].childNodes[5].childNodes[0].value += ref + "\n"
}
...
function showResponse(responseText, statusText, xhr, $form) {
  if (responseText.indexOf("errorMessage") == -1) {
    ...
    closeQuickReply();
    if (hasCaptcha == 1) Recaptcha.reload("t")
  } else {
...
function expandImage(thumbLink) {
  ...
  if (image.className.indexOf("expandedThumb") == -1) {
    ...
    if (image.className.indexOf("opThumb") == -1) {
      ...
      image.onload = function () {
        if (dicks) {
          image.style.width = image.naturalWidth * 0.504 + "px";
          image.style.height = image.naturalHeight * 0.504 + "px";
          dicks = 0
        }
      };
      if (window.pageYOffset > findPosY(image)) window.scroll(0,
      findPosY(image) - 75)
    } else {
      image.className = "thumb opThumb";
      if (window.pageYOffset > findPosY(image)) window.scroll(0, findPosY(image) - 75)
    }
  }
}
...
function quotePreview(reference, mode) {
  ...
  if (document.getElementById("parent" + referenceWork.substring(8)) == null) var referencedPostNumber = "reply" + referenceWork.substring(8);
  else var referencedPostNumber = "parent" + referenceWork.substring(8);
  ...
  if (document.getElementById(referencedPostNumber) != null) {
    if (mode == 0) {
      ...
      if (post.offset().top + post.height() >= $(window).scrollTop() && post.offset().top <= $(window).scrollTop() + $(window).height()) if (post.attr("class") == "reply") post.attr("class", "reply highlight");
      ...
      $(reference).mousemove(function (e) {
        ...
        if ($(div).width() + (e.pageX + 50) >= $(window).width()) div.style.width = $(div).width() - ($(div).width() + e.pageX + 50 - $(window).width()) + "px"
      });
      if (document.getElementById(div.id) == null) {
        $(div).find("div[id^=inlineQuote]").remove();
        $(div).find("div[id^=backlinkInlineQuote]").remove();
        if ($(div).find("a.thumbLink").children("img.expandedThumb").length) $(div).find("a.thumbLink").each(function (e) {
          expandImage(this)
        });
    ...
    if (mode == 1) {
      if (post.attr("class") == "reply highlight") post.attr("class", "reply");
    ...
    if (mode == 0) {
      ...
      $.getJSON(json + ".json", function (data) {
        $.each(data.posts, function (index, post) {
          if (post.no == referencedPostNumber.replace("reply", "")) $(div).html($(makeReply(post)).children(".reply").last().html())
        });
      ...
      if (document.getElementById(div.id) == null) document.body.appendChild(div)
    }
    if (mode == 1) document.body.removeChild(document.body.lastElementChild)
  }
}
...
function inlineQuote(reference, url, mode, backlink) {
  ...
  if (backlink == 1) var alreadyQuote = $(reference.parentElement.parentElement.parentElement).find("#backlinkInlineQuote" + referencedPostNumber).attr("id");
  if (document.getElementById(referencedPostNumber) != null) if (alreadyQuote == undefined) {
    ...
    if ($("body").find("#quotePreview" + referencedPostNumber).length) $(div).removeClass("highlight");
    if (backlink == 1) if ($(reference).nextUntil("blockquote").length == 0) {
      $(div).attr("id", "backlinkInlineQuote" + referencedPostNumber);
      $(reference).after(containerDiv)
    } else {
      $(div).attr("id", "backlinkInlineQuote" + referencedPostNumber);
      $(reference).nextUntil("blockquote").last().after(containerDiv)
    } else $(reference).after(containerDiv)
  } else {
    ...
    if (backlink == 1) $("#backlinkInlineQuote" + referencedPostNumber).parent().remove();
    else $($(reference).next()).remove()
  } else if (alreadyQuote == undefined) {
    var json = reference.href.replace(/#.*/i, "");
    json = json.replace(/.html/i, "");
    $.getJSON(json + ".json", function (data) {
      $.each(data.posts, function (index, post) {
...
function threadUpdater() {
  ...
  modified = req.getResponseHeader("Last-Modified");
  json = json + ".json";
  if (document.getElementById("lastModified").innerHTML == modified) console.log("No new posts");
  else {
    ...
    $.getJSON(json, function (data) {
      ...
      var newPosts = [];
      $.each(data.posts, function (index, post) {
        if (post.no > lastPost) newPosts.push(makeReply(post))
      });
      if (postsInTitle == 0) {
  ...
  updaterTimer = setTimeout("threadUpdater()",
  15E3)
}
...
function updaterCounter() {
  ...
  updaterTimeLeft = setTimeout("updaterCounter()", 1E3)
}
...
function makeReply(post) {
  ...
  if (post.filename) {
    ...
    if (filename.length > 25) filename = filename.substring(0, 25) + "(...)" + post.filename.substring(post.filename.lastIndexOf("."));
    else filename = post.filename;
    fileSize.innerHTML = "File: <a target='_blank' href='" + boardPath + post.image + "'>" + filename + "</a> -(" + Math.round(post.fsize / 1E3) + " KB, " + post.w + "x" + post.h + ")";
    ...
    thumbnail.setAttribute("style", "width:" + post.tn_w * 0.504 + "px;height:" + post.tn_h * 0.504 + "px;");
...
function birthday(birthday, play) {
  if (birthday == 0) {
    for (var i = 0; i < document.getElementsByClassName("hat").length; i++) document.getElementsByClassName("hat")[i].innerHTML = "<img src='" + domain + "img/partyhat.gif' style='position:absolute; margin-top:-100px;'/>";
  ...
  if (birthday == 1) {
    ...
    window.setTimeout("birthday(1,0)",
    30);
    ...
    for (var i = 0; i < document.getElementsByClassName("reply").length; i++) document.getElementsByClassName("reply")[i].style.background = ColorValue
  }
}
